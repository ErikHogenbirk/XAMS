# Update Oct. 5: update of ZeroFieldProperties
...
class ZeroFieldProperties(hax.minitrees.TreeMaker):
    '''
    This treemaker is especially made for zero-field data. It assumes that all the interesting
    peaks are classified as S1 (i.e. you might want to use the GenerousS1Classification). 
    '''
    __version__ = '0.0.0'
    extra_branches  = ['peaks.*', 'interactions*']
    ...
    def extract_data(self, event):
        peak_properties = ['n_saturated_channels', 'center_time', 'left', 'right', 'area_midpoint',
                           'area', 'area_fraction_top', 'height'
                           ]

        # Peak properties for S1
        extra_properties = ['time_from_midpoint_10p', 'time_from_midpoint_20p',
                            'time_from_midpoint_30p', 'time_from_midpoint_40p',
                            'range_20p_area', 'range_50p_area', 'range_80p_area'
                           ]
        result_vars = ['s1_' + _pp for _pp in (peak_properties + extra_properties) ]
        result_vars = result_vars + ['n_s1s', 'largest_other_s1']
        result = {k: float('nan') for k in result_vars}
        
        # Make list of all S1s in the event
        s1s = []
        for peak in event.peaks:
            if peak.type == 's1':
                s1s.append(peak)
        # Store the number of s1s and check if we have an S1
        n_s1s = len(s1s)
        result['n_s1s'] = n_s1s
        if n_s1s == 0:
            return result
        ...
        # Extract all standard properties
        for prop in peak_properties:
            result['s1_' + prop] = getattr(s1, prop)
        # Here come nonstandard properties
        for i in range(10):
            if ('range_%d0p_area' % i) in extra_properties:
                result['s1_range_%d0p_area' % i] = (
                    list(s1.range_area_decile)[i])
            if ('time_from_midpoint_%d0p' % i) in extra_properties:
                result['s1_time_from_midpoint_%d0p' % i] = (
                    list(s1.area_decile_from_midpoint)[i])
        
        # Largest other S1 properties
        if n_s1s >= 2:
            result['largest_other_s1'] = getattr(s1s[1], 'area')
        else:
            result['largest_other_s1'] = 0.
        return result
...
class S1PulseZeroField(hax.minitrees.TreeMaker):
    '''
    Retrieves the S1 waveform, but for zero field data (i.e. the LARGEST S1, not the s1 of the main interaction...). Use pickle as format for this one.
    '''
    __version__ = '0.0.2'
    extra_branches = ['*']
